...
مدیریت دانش شخصی مجموعه‌ای از فعالیت‌هاست که یک نفر برای جمع‌آوری، سازماندهی، به کارگیری و بازیابی دانش خود به کار می‌گیرد.
...
فرایندی که من برای مدیریت دانش استفاده می‌کنم شامل چهار مرحله[^3] است:
...
در این هرم چهار لایه وجود دارد: داده‌، اطلاعات، دانش و خرد. در فرایند چهار مرحله‌ای تلاش می‌کنیم داده‌های دریافتی را ثبت کنیم. با سازماندهی و معنا بخشی آنها را به اطلاعات تبدیل کنیم. با پردازش و به کارگیری اطلاعات را به دانش تبدیل کنیم. با خلق کردن و بیان کردن دانسته‌ها را عمیق‌تر کنیم و به خرد برسیم.[^4]
...
در این مرحله داده‌ها را دریافت و ذخیره می‌کنیم. این داده‌ها شامل هرچیزی است که شما دریافت می‌کنید و بعدا نیازی دارید آنها را بازیابی کنید؛ کتابی که مطالعه کردید، پادکستی که گوش کردید، ویدئوی که مشاهده کردید و هر محتوایی که از هر منبعی مصرف کردید.

برای ذخیره سازی داده‌ها می‌توانیم از [[note-taking|یادداشت برداری]] استفاده کنیم. وقتی چیزی را می‌نویسیم، آن را به بند می‌کشیم و اجازه نمی‌دهیم به سادگی از ذهنمان فرار کند. اینکه فقط به مطالعه بسنده کنیم و امیدوار باشیم مطالب در ذهنمان باقی بمانند، روش مطمئنی نیست و معمولا نتیجه نمی‌دهد. 

علاوه بر این داده‌های دیگری هم هستند که باید آنها را ذخیره کنید؛ مثل گزارش‌ها، فاکتور‌ها، پروژه‌ها، تسک‌ها، مسئولیت ها و اقدامات. این موارد در واقع اطلاعات شخصی یا خصوصی شما است و ارتباط مستقیمی با دانش ندارد اما چون مرز شفافی بین این دسته از اطلاعات و دانش وجود ندارد همه‌ی این‌ها در یک سیستم واحد ذخیره می‌شوند.[^5]
...
این جمله استعاره‌ای از **کشف حقیقت بر پایه‌ی اکتشافات قبلی** است. یعنی دانسته‌های پیشین ما زمینه‌ای فراهم می‌کنند تا یادگیری‌مان عمیق‌تر و مؤثرتر باشد. [^6]
...
این سیستم توسط فیلسوف آلمانی نیکلاس لومان طراحی شده است. [[zettelkasten|زتلکاستن]] یک سیستم شبکه‌ای برای یادداشت‌برداری و تفکر عمیق است.
...
[[digital-garden|دیجیتال گاردن]] روشی است که بر اساس استعاره‌ی باغبانی شکل گرفته است. تصور کنید یک زمین حاصل‌خیز دارید که بذر دانش را در آن می‌کارید. محتوایی که مطالعه می‌کنید همچون خاک بستر رشد بذر‌ها را فراهم می‌کند. هر بذری که می‌کارید در این خاک ریشه دوانده و از آن تغذیه می‌کند.
...
[[basb|ساختن مغز دوم]] توسط تیاگو فورته طراحی شده است.  هدف این سیستم بیرون آوردن اطلاعات از ذهن و ذخیره‌کردن آن‌ها در یک سیستم دیجیتال است. این روش به شما کمک می‌کند ایده‌ها، یادداشت‌ها، پروژه‌ها و منابع مختلف را به‌صورت منظم و ساختاریافته نگه دارید تا هر زمان که نیاز داشتید، سریع به آن‌ها دسترسی پیدا کنید.
...
[^3]: این چهار مرحله در واقع همان فرایند چهار مرحله‌ای CODE است که توسط تیاگو فورته طراحی شده اما من ترجیح دادم در مرحله سوم به جای عصاره گیری(Distill) از پردازش استفاده کنم. در این مرحله من اقدامات بیشتری انجام می‌دهم که واژه‌ی عصاره گیری شامل آنها نمی‌شود.

[^4]:  لزوما قرار نیست همه‌ی داده‌ها این مراحل را طی کنند. بعضی از منابع اصلا ارزش این همه وقت گذاشتن ندارند. ممکن است بعضی از داده‌ها در مرحله اطلاعات باقی بمانند. همه مطالب قرار نیست به بالاترین سطح برسند. به قول فرانسیس بیکن: «بعضی از کتاب‌ها را باید چشید، بعضی دیگر را باید بلعید، و تعداد کمی را باید جوید و هضم کرد؛ یعنی بعضی از کتاب‌ها را باید فقط در بخش‌هایی خواند؛ بعضی دیگر را باید خواند، اما نه با کنجکاوی؛ و تعداد کمی را باید به‌طور کامل، با پشتکار و توجه خواند.» ([+](https://www.goodreads.com/quotes/1242120-some-books-are-to-be-tasted-others-to-be-swallowed))

[^5]: در حقیقت این موارد تحت عنوان **مدیریت اطلاعات شخصی (Personal Information Management)** قرار می‌گیرند و سیستم مجزای خود را می‌طلبند. مثلا سیستم GTD بیشتر مربوط به همین اطلاعات شخصی است. در مقابل سیستم زتلکاستن تمرکز بیشتری روی دانش دارد. اما سیستم مغز دوم ترکیبی از هردوی این‌هاست هم شامل دانش است هم اطلاعات شخصی.

[^6]: احتمالا شما هم در کلاس‌های آموزشی دیده‌اید؛ برخی سریع مطلب را درک می‌کنند و از توضیح دوباره استاد کلافه می‌شوند، درحالی که برخی دیگر گیج می‌زنند و مدام سوال می‌پرسند. این تفاوت از کجا می‌آید؟ شاید فکر کنید نفر اول باهوش و نفر دوم خنگ است. اما یک احتمال دیگر هم وجود دارد و آن **پیشینه یادگیری** است. (کتاب یادگیری زایا، ص19)

[^7]: مرحوم مظفر در کتاب المنطق از اصطلاح رأس المال برای توصیف این وضعیت استفاده می کند: «ضروریات (بدیهیات) خودبه‌خود در اختیار انسان هستند و این‌ها رأس‌المال اصلی او برای کسب علوم نظری‌اند. انسان با این سرمایه، مفاهیم جدید را فرا می‌گیرد. هرچه بیشتر بیاموزد، رأس‌المالش بیشتر می‌شود و در نتیجه توان بیشتری برای یادگیری مطالب جدید پیدا می‌کند؛ همان‌طور که تاجر با افزایش سرمایه‌اش، توان سودآوری بیشتری پیدا می‌کند.» ([المنطق، جلد1، صفحه 24](https://lib.eshia.ir/10463/1/24))
