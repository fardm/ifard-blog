...
لینک ریپازتوری کوارتز من اینه:

https://github.com/fardm/ifard-blog-v5

فایل هایی که ارجاع میدم رو میتونید از اینجا پیدا کنید.
...
### رابط کاربری
خلاصه استایل هایی که توی `quartz/styles/_ui.scss` اضافه کردم:
...
```tsx title="quartz/components/frames/DefaultFrame.tsx"
<div class="center">
  <div class="page-top"> // این
...
برای استفاده از این استایل فایل `_ui.scss` رو دانلود و در مسیر `quartz/styles` قرار بدید. بعد کد زیر رو به ابتدای فایل `custom.scs` اضافه کنید.
```scss
@use "./_ui.scss";
```
...
### ‌کال‌اوت
توی فایل `quartz/styles/_my-callouts.scss` چندتا کال‌اوت بدون آیکون با رنگ های مختلف ساختم:
...
برای اضافه کردن کافیه فایل `_card-view.scss` دانلود رو در مسیر `quartz/styles` قرار بدید. بعد کد زیر رو به ابتدای فایل `custom.scs` اضافه کنید.
```scss
@use "./_card-view.scss";
```
...
کد های این گرید توی فایل `quartz/styles/_my-callouts.scss` با این عنوان مشخص شدند:
```
// ================ 02. GRID LAYOUTS ================
```
...
این استایل هم توی فایل `quartz/styles/_my-callouts.scss` اضافه شده:
```
// ================ 03. IMGAGES CAPTION  ================
```
...
### کانتنت متا پلاس (Content MetaPlus)
کامپوننت ContentMeta تاریخ و مدت زمان مطالعه رو ابتدای یادداشت نشون میده. برای اینکه بتونم این بخش رو شخصی کنم یک پلاگین جدید به اسم `ContentMetaPlus` ساختم. خلاصه مواردی که اضافه کردم ایناست:
...
### لایت باکس تصاویر (Clickable Images)
یکی از کاربرا برای نسخه‌ی قبل پلاگینی نوشته بود که تصاویر رو قابل کلیک کرده بود تا لایت باکس داشته باشه. فایلش اینجاست:
...
### کامنت (standalone comments)


من قبلا از giscus استفاده میکردم. مشکلش این بود که فقط کسی که  اکانت گیت هاب داشت میتونست کامنت بذاره. به خاطر همین تعامل خیلی کم شده بود. ایمپورت و اکسپورت هم نداشت. بعد از اینکه سایت رو به هاست اشتراکی منتقل کردم یه روش پیدا کردم که به سایت های استاتیک کامنت اضافه میکنه.

ریپازتوری اصلی اینجاست: [standalone-comments](https://github.com/dlnorman/standalone-comments)

برای اینکه بشه توی کوارتز ازش استفاده کرد این ریپو رو فورک کردم و یه تغییراتی توش دادم. فایل ها و آموزش استفاده ازش اینجاست:
